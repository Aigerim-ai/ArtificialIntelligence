...
def UCSearch(start, goal, barrier):
    ...
    while len(fringe) > 0:
        ...
        par=get_neighbors(curNode)
        for neighbor in par:
            neighborcost=move_cost(neighbor,barrier)
            befortotal=G[curNode]
            total=befortotal+neighborcost
            if neighbor in fringe or neighbor in expandedNodes:
                if total< G[neighbor]:
                    G[neighbor]=total
...
def aStarSearch(start, goal, barrier):
    ...
    while len(fringe) > 0:
        ...
        for neighbor in par:
            ...
            befortotal=G[curNode]
            total=befortotal+neighborcost
            heurofneigh=heuristic(neighbor,goal)
            totalwithheur=heurofneigh+total
            if neighbor in fringe or neighbor in expandedNodes:
